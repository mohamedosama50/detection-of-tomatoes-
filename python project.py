...
img=cv2.imread("88.jpg")# Reading the image
...
tot=cv2.bitwise_or(res_r,res_g) # ORing res_r with res_g (Mix)
res_r1 =cv2.cvtColor(res_r,cv2.COLOR_BGR2RGB)
res_g1 =cv2.cvtColor(res_g,cv2.COLOR_BGR2RGB)
...
titles =['original image','red detection','green detection','Mix','full detection']
images=[imgage,res_r1,res_g1,tot,img2]
for i in range(5):
